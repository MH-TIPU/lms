    ...
    if ($(window).width() <= 991) {
        setTimeout(function () {
            if ($(".campaign").css("display") == 'block') {
                $('main').removeClass('mrt-150 mrt-205 mrt-110').addClass('mrt-170');
            } else {
                $('main').removeClass('mrt-150 mrt-205 mrt-170').addClass('mrt-110');
            }
        }, 100);
    } else {
        setTimeout(function () {
            if ($(".campaign").css("display") == 'block') {
                $('main').removeClass('mrt-150 mrt-170 mrt-110').addClass('mrt-205');
                $(".sidebar-sticky").css("top", "155px");
            } else {
                $('main').removeClass('mrt-205 mrt-170 mrt-110').addClass('mrt-150');
...
$(window).scroll(function () {
    $(this).scrollTop() >= offset ? ($("#navigation").addClass("sticky"), $(".scrollToTop").fadeIn()) : ($("#navigation").removeClass("sticky"), $(".scrollToTop").fadeOut());
});
...
$(".search-icon").on("click", function () {
    ...
    $(".t-header-search").toggleClass("is-active");
});
...
$(document).on("click", function (s) {
    if (!$(s.target).closest("#navigation, .menu-icon").length) {
...
$(".episodes-list-group-head").on("click", function () {
    ...
    s.hasClass("open") ? (s.removeClass("open"), $(this).parent().find(".episodes-list-group-body").slideUp(200)) : (s.addClass("open"), $(this).parent().find(".episodes-list-group-body").slideDown(200));
});
...
$(".moon").on("click touchstart", function () {
...
function toastNotifi(message, bg, icon, delay = 3000) {
    ...
    $('.toast').removeClass('bg-toast-info bg-toast-success bg-toast-alert').addClass(bg);
    $('.toast__icon').removeClass('toast-info toast-success toast-alert').addClass(icon);
    $('.toast').fadeIn(200).delay(delay).fadeOut(400);
}

$(".episodes-list-item.lock").click(function (e) {
    e.preventDefault();
    toastNotifi('To view the course, you must first purchase it', 'bg-toast-info', 'toast-info', 2000);
});
